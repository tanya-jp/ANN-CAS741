...
\item [\refstepcounter{mnum} \mthemnum \label{Train}:] Training Module
...
\begin{description}
  \item[Secrets:]Preparing the input data in a format that can be used by model for classifying.
...
\begin{description}
  \item[Secrets:]Preparing training dataset in a format that can be used by model for training and testing.
...
\subsubsection{Training Module(\mref{Train})}
...
\begin{description}
  ...
  from \mref{Train-Model} to find the best weights between neurons and each neuron's biase.
  \item[Implemented By:] ANN
  % \item[Type of Module:] Abstract Data Type
\end{description}
...
R4 & \mref{HW}, \mref{ACM}, \mref{SavedANN}, \mref{Output}, \mref{Feedback}\\
